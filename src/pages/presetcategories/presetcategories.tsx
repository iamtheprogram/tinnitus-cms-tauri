...
const PresetCategories: React.FC = () => {
    ...
    const categories = useSelector<CombinedStates>((state) => state.presetReducer.categories) as PresetCategory[];
    ...
    async function onAddCategory(): Promise<void> {
        ...
        if (counter === 0) {
            try {
                ...
                if (type === 'Add') {
                    await addAlbumCategory(data, 'presets');
                    ...
                    await editAlbumCategory(data, 'presets');
                    const temp = categories.map((x: any) => x);
    ...
    async function onCategoryDelete(it: number): Promise<void> {
        try {
            await deleteAlbumCategory(categories[it], 'presets');
            const temp = categories.map((x: any) => x);
            temp.splice(it, 1);
            dispatch({
                type: 'preset/categories',
    ...
    return (
        <div className="page" id="page-upload-edit">
            ...
            <div className="categories-section">
                <h2 className="categories-header">Preset categories</h2>
