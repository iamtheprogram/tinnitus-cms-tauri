pub mod oci_provider {
    ...
    pub fn init_provider(provider: OciProvider) -> Result<(), Box<dyn std::error::Error>> {
        STORE.put("user".to_string(), &provider.user)?;
        STORE.put("tenancy".to_string(), &provider.tenancy)?;
        STORE.put("fingerprint".to_string(), &provider.fingerprint)?;
        STORE.put("namespace".to_string(), &provider.namespace)?;
        STORE.put("preauthreq".to_string(), &provider.preauthreq)?;
        // let pem = parse(provider.key).unwrap().contents;
        // STORE.put("key".to_string(), &pem)?;
        // STORE.put("key_id".to_string(), &provider.key_id)?;

        let user: Option<String> = STORE.get("user".to_string())?;
        assert_eq!(user.unwrap(), provider.user);
        let tenancy: Option<String> = STORE.get("tenancy".to_string())?;
        assert_eq!(tenancy.unwrap(), provider.tenancy);
        let fingerprint: Option<String> = STORE.get("fingerprint".to_string())?;
        assert_eq!(fingerprint.unwrap(), provider.fingerprint);
        let namespace: Option<String> = STORE.get("namespace".to_string())?;
