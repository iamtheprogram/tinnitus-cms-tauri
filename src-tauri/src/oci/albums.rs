pub mod albums_utils {
    use crate::{
        http_utils::http_utils::HttpService,
        oci_obj_storage::oci_obj_storage::OciObjectStorageServices,
        oci_provider::oci_provider::STORE,
    };
    ...
    impl AlbumsService {
        pub fn new() -> Self {
            let user: String = STORE.get("user".to_string()).unwrap().unwrap();
            let tenancy: String = STORE.get("tenancy".to_string()).unwrap().unwrap();
            let fingerprint: String = STORE.get("fingerprint".to_string()).unwrap().unwrap();
            let namespace: String = STORE.get("namespace".to_string()).unwrap().unwrap();
            let preauthreq: String = STORE.get("preauthreq".to_string()).unwrap().unwrap();
            Self {
                user,
                tenancy,
                fingerprint,
