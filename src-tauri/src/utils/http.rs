pub mod http_utils {
    ...
    use crate::oci_provider::oci_provider::STORE;
    ...
    impl HttpService {
        ...
        pub fn generate_request_get(&mut self) -> Result<(), Box<dyn std::error::Error>> {
            ...
            //Append headers
            self.append_headers("GET".to_string(), None)
                .unwrap();
            //Create signature
            let signature = self.generate_signature().unwrap();
            //Create authorization header
        ...
        fn append_headers(
            ...
            self.headers.append("host", HeaderValue::from_str(&uri_host).unwrap());
            self.headers_to_sign
                .insert("host".to_string(), uri_host.to_string());

            let date = fmt_http_date(SystemTime::now());
            self.headers.append("Date", HeaderValue::from_str(&date).unwrap());
            self.headers_to_sign.insert("date".to_string(), date.clone());

            // let x_date = Utc::now().to_rfc2822();
            self.headers.append("x-date", HeaderValue::from_str(&date).unwrap());
            ...
            if method == "PUT" || method == "POST" {
                if let Some(data) = data {
                    ...
                    self.headers.append("Content-type", HeaderValue::from_static("application/json"));
                    self.headers_to_sign
        ...
        fn generate_signature(&self) -> Result<String, Box<dyn std::error::Error>> {
            ...
            string_to_sign
                .push_str(format!("(request-target): {} {}\n", &method, &target).as_str());
        ...
        fn set_auth_header(&mut self, signature: &String) -> Result<(), Box<dyn std::error::Error>> {
            ...
            auth_header.push_str(format!("{}", key_id).as_str());
            auth_header.push_str(r#"","#);
            auth_header.push_str(format!(r#"algorithm="rsa-sha256","#).as_str());
            let mut headers_string = String::new();
            for (key, _value) in &self.headers_to_sign {
            ...
            auth_header.push_str(format!("{}", signature).as_str());
            auth_header.push_str(r#"""#);
            //Append auth header
            self.headers.append("Authorization", HeaderValue::from_str(&auth_header).unwrap());
